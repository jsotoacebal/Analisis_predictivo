...
ONPL$data_channel = factor(ONPL$data_channel,
                      levels = c("Others","Tech","World","Business","Social Media","Lifestyle"))
...
ONPL_sum_channel = ONPL %>% group_by(data_channel) %>% summarise(suma_dc = sum(shares)) %>% arrange(desc(suma_dc)) %>% filter(!is.na(data_channel))
ONPL_sum_channel

ONPL_sum_channel$data_channel = factor(ONPL_sum_channel$data_channel,
                           levels = c("Others","Tech","World","Business","Social Media","Lifestyle"))


ggplot(ONPL_sum_channel, aes(x = data_channel, suma_dc, fill = data_channel)) +
